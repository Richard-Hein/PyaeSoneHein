...
const MyJourney = () => {
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 gap-2 max-w-[1000px] mx-auto mt-[80px]">
       <div className="mx-4">
      ...
      {myEducationData.map((item, index) => (
        <div key={index} className="mb-10 relative border rounded-lg shadow hover:border-primary hover:shadow-2xl cursor">
          {/* Dot */}
      ...
      {myDeveloperData.map((item, index) => (
        <div key={index} className="mb-10 ml-0 md:ml-6 relative border rounded-lg shadow hover:border-primary hover:shadow-2xl ">
          {/* Dot */}
          <span className="absolute w-4 h-4 shadow-lg bg-primary rounded-full -left-2 top-[-7px]" />
          <div className="bg-white p-4 rounded-md shadow-md">
            <h3 className="text-lg font-semibold">{item.title}</h3>
